...
//			addY=250;
			...
			if(gx<200){
				if(hua[20]>1){
					Toast.makeText(xinlvActivity.this, "请用您的指尖盖住摄像头镜头！", Toast.LENGTH_SHORT).show();
					hua[20]=0;}
				hua[20]++;
				return;}
			else
	...
	private static PreviewCallback previewCallback = new PreviewCallback() {
		...
		public void onPreviewFrame(byte[] data, Camera cam) {
			...
			gx=imgAvg;
			text1.setText("平均像素值："+String.valueOf(imgAvg));
			//像素平均值imgAvg,日志
			...
			if (imgAvg < rollingAverage) {
				...
				if (newType != currentType) {
					beats++;
					flag=0;
					text2.setText("脉冲数："+String.valueOf(beats));
...
//获取系统结束时间（ms）
			...
			if (totalTimeInSecs >= 2) {
				...
				text.setText("心率是"+String.valueOf(beatsAvg)+"  zhi:"+String.valueOf(beatsArray.length)
